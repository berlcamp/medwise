...
import { ChevronDown, CreditCard, Eye, FileText, Plus, Printer } from 'lucide-react'
...
export default function AgentDashboardPage() {
  ...
  useEffect(() => {
    const loadAgentData = async () => {
      ...
      try {
        ...
        // Match by transaction_type='agent_sale' and customer_name matching agent name
  ...
  const totalItemsAdded = agentItems.reduce((sum, i) => sum + i.quantity_added, 0)
  const totalItemsSold = agentItems.reduce((sum, i) => sum + i.quantity_sold, 0)
  const totalItemsReturned = agentItems.reduce((sum, i) => sum + i.quantity_returned, 0)
  const totalCurrentBalance = agentItems.reduce((sum, i) => sum + i.current_balance, 0)
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-white shadow-lg rounded-xl p-12 text-center border border-gray-100">
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Agent Not Found</h3>
            <p className="text-gray-500">No active agent record found for your account.</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 bg-gray-50 rounded-lg">
          <div>
            ...
            <p className="text-xl font-bold text-green-900">{totalItemsAdded}</p>
          </div>
          <div>
            <p className="text-xs text-gray-600">Items Sold</p>
            <p className="text-xl font-bold text-orange-900">{totalItemsSold}</p>
          </div>
          <div>
            <p className="text-xs text-gray-600">Items Returned</p>
            <p className="text-xl font-bold text-purple-900">{totalItemsReturned}</p>
          </div>
          <div>
            <p className="text-xs text-gray-600">Current Balance</p>
            <p className="text-xl font-bold text-gray-900">{totalCurrentBalance}</p>
          </div>
          <div>
                  ...
                  {agentItems.length === 0 ? (
                    ...
                    agentItems.map((item: AgentItem) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-center">
                          <span className="font-semibold">{item.current_balance}</span>
                        </TableCell>
              ...
              <table className="app__table">
                ...
                <tbody>
                  {transactions.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="app__td text-center text-gray-500">
                      ...
                      <tr key={item.id} className="app__tr">
                        ...
                        <td className="app__td text-right">
                          <span className="font-semibold text-gray-900">
                            ₱{Number(item.total_amount || 0).toLocaleString('en-US', {
                        ...
                        <td className="app__td text-center">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => printDeliveryReceipt(item)}>
