 ...
 */
export async function addConsignmentItems(
  params: AddConsignmentItemsParams
): Promise<{
  success: boolean;
  message?: string;
  items_added?: number;
...
export async function generateTransactionNumber(
  branch_id: number,
  transactionType = "consignment_sale"
): Promise<string> {
  try {
    ...
    if (error) {
      ...
      return `${transactionType || "txn"}-${Date.now()}`;
    }

    return data || `${transactionType || "txn"}-${Date.now()}`;
  } catch (err) {
